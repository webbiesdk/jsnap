(function() {
    ...
    function dump(obj, seenFuncs) {
        ...
        for (var i=0; i<props.length; i++) {
            var prop = props[i];
            if (prop.substring(0,'__jsnapHiddenProp__'.length) === '__jsnapHiddenProp__')
                continue;
            if (prop === '__proto__')
                continue;
            try {
        ...
        if (hasPrty(obj, '__jsnapHiddenProp__recordedCalls')) {
            recordedCallsQueue.push(function () {
                objDump.recordedCalls = convertValue(obj.__jsnapHiddenProp__recordedCalls, true);
            });
        }
    ...
    function convertFun(fun, value, seenFuncs) {
        ...
        if (seenFuncs !== true && createInstances && fun.type == "user" || fun.type == "bind") {
