(function(options) {
    ...
    Function.prototype.apply = function (that, args) {
        var func = this;
        args = args || [];
        return window.__jsnapHiddenProp__recordArguments.__jsnapHiddenProp__call(that, func, args);
    };
    ...
    defineHiddenProperty(window, "__jsnapHiddenProp__recordArguments_method", function(prop, arguments, isNew) {
        var obj = this;
        return window.__jsnapHiddenProp__recordArguments.__jsnapHiddenProp__call(obj, obj[prop], arguments, isNew)
    });

    defineHiddenProperty(window, "__jsnapHiddenProp__recordArguments", function(func, arguments, isNew) {
        var type = func.__jsnapHiddenProp__fun.type;
        var that = this;
        if (isNew) {
            that = Object.create(func.prototype);
        }
        var result;
        if (type != "user" && type != "bind") {
            if (isNew) {
                result = func.__jsnapHiddenProp__apply(that, arguments);
                ...
                return func.__jsnapHiddenProp__apply(that, arguments);
            }
        }
        var records;
